...
module Swagger =
  ...
  //and PropertyDefinitionOptionConverter() =
  //  inherit JsonConverter()
  //    override __.WriteJson(writer:JsonWriter,value:obj,_:JsonSerializer) =
  //      let po = unbox<PropertyDefinition option>(value)
  //      match po with
  //      | Some p ->
  //          writer.WriteStartObject()
  ...
  //              writer.WritePropertyName "$ref"
  //              writer.WriteValue (sprintf "#/definitions/%s" ref.Id)
  ...
  //      unbox ""
  //    override __.CanConvert(objectType:Type) = 
  //      objectType = typeof<PropertyDefinition option>
  and ParamDescriptorConverter() =
    inherit JsonConverter()
        override __.WriteJson(writer:JsonWriter,value:obj,_:JsonSerializer) =
          let p = unbox<ParamDescriptor>(value)
          //writer.WriteStartObject()
          writer.WriteNull()
          //writer.WriteEndObject()
          writer.Flush()
        override __.ReadJson(_:JsonReader,_:Type,_:obj,_:JsonSerializer) =
          unbox ""
        override __.CanConvert(objectType:Type) = 
          objectType = typeof<ParamDescriptor>
  and ParamDefinitionConverter()=
    inherit JsonConverter()
        override __.WriteJson(writer:JsonWriter,value:obj,_:JsonSerializer) =
          ...
          //writer.WriteStartObject()
          writer.WriteRawValue (p.ToJson())
          //writer.WritePropertyName "popo"
          //writer.WriteValue "lala"
          //writer.WriteEndObject()
          writer.Flush()
        override __.ReadJson(_:JsonReader,_:Type,_:obj,_:JsonSerializer) =
          unbox ""
      ...
      fun ctx -> 
        ...
        let streamZipContent () =
          ...
          let disposeStreams() =
            fs.Dispose()
            zip.Dispose()
          match findInZip zip (fun e -> e.FileName = p) with
          | Some _ -> 
            let headers = 
            ...
            if p = "index.html"
            then
              let url = sprintf "http://%s:%d/" ctx.request.host ctx.request.url.Port
              use r = new StreamReader(zip)
              let bytes = 
                r.ReadToEnd()
                  //.Replace("http://petstore.swagger.io/v2/swagger.json", (combineUrls url swJsonPath))
              ...
              { ctx
                  with 
                    response = 
                      { ctx.response 
                          with 
                            status = Suave.Http.HttpCode.HTTP_200.status
                            content = Bytes bytes
                            headers = headers
                      }
              } |> succeed
